...
4. **Gap Verification:** Four structured outputs (Coverage Matrix, Assumption Register, Scenario Matrix, Cross-Cutting Concern Sweep) plus 10 automated lint rules.

The audit gate is informational, not blocking. A GREEN score with all gaps closed means "ready to build." A YELLOW score with gaps means "proceed with caution." RED means "go back to Phase 3 or 4." The decision to proceed is the user's, but the data is objective.
...
Test is the only phase with a hard gate. Before proceeding to Handoff, all of these must be true:
...
```text
 ...
 │  │  │                                                         │  │  │
 │  │  │  Phase 5: Audit scores plan across 8 dimensions        │  │  │
 │  │  │  Phase 7: Impact Review checks cross-cutting concerns  │  │  │
 │  │  │  Phase 8: Test gate - hard pass/fail before handoff    │  │  │
 │  │  │                                                         │  │  │
 │  │  └─────────────────────────────────────────────────────────┘  │  │
 │  │                                                               │  │
...
**Phase 5 (Audit)** scores the plan across 8 quality dimensions, using a 1-5 scale per dimension. Thresholds: 32-40 GREEN, 24-31 YELLOW, 16-23 ORANGE, 1-15 RED. The audit also runs a devil's advocate challenge ("if this fails in production, what is the most likely reason?") and verifies that file paths and function names referenced in the plan actually exist.
...
**Phase 8 (Test)** is the only phase with a hard pass/fail gate. All critical path tests must pass, no open P0/P1 defects, characterization baselines captured for refactored code, audit score at GREEN or YELLOW, and rollback plan validated. If any condition fails, the plan stays in Test. There is no override. A test gate that can be bypassed is not a gate; it is a suggestion.
...
On top of the 4 matrices, 10 binary pass/fail rules run as automated checks. These are the plan equivalent of a linter. Either the rule passes or it does not.

| Rule | Description |
| :----- | :------------ |
| LINT-01 | Every goal has a mapped ticket |
| LINT-02 | Every HIGH risk has a mitigation |
| LINT-03 | Every deployment has a rollback |
| LINT-04 | Every external dependency has an owner |
| LINT-05 | Every new endpoint has a contract/test |
| LINT-06 | Backward compat has a mixed-state scenario |
| LINT-07 | Every new behavior has a test delta |
| LINT-08 | No unverified HIGH-impact assumptions |
| LINT-09 | No unaddressed cross-cutting concern |
| LINT-10 | Every phase has go/no-go criteria |

A plan is "gap-checked" only when all 10 lint rules pass, the Coverage Matrix has zero gaps, no unverified HIGH-impact assumptions remain, the Scenario Matrix has zero gaps, and the Cross-Cutting Sweep has zero gaps. That is a high bar. Most first-draft plans fail it. That is the point. The audit shows you exactly what to add.
...
graph TD
    ...
    A3 -->|"Dims 6, 8"| VERIFY
    A4 -->|"Dim 7"| VERIFY
    A5 -->|"4 gap artifacts<br/>+ 10 lint rules"| VERIFY
...
- [OpenAI: Harness Engineering](https://openai.com/index/harness-engineering/) - The principle that architectural constraints should be enforced mechanically, not by suggestion. The 10 lint rules implement this principle for plans.
