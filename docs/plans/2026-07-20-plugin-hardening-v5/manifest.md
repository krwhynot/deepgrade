...
| [CR-3](changes/CR-3.md) | 2026-07-29 | **PROPOSED — unratified, nothing applied.** (Deliberate wording: the sweep greps `manifest.md` file-wide for pre-acceptance phrasing to catch stale CR-1 text, so a genuinely-open CR must state its status without those exact phrases. The authoritative state lives in `changes/CR-3.md` and `status.json`.) Would extend acceptance row A1 from `*.sh` to also cover `*.js`: G0 selected lane N, so the hook guards are now node scripts, and `.gitattributes`'s own rationale ("the hook guards and the test suite are shell scripts") is no longer true of the tree. Severity deliberately marked LOWER than CR-2's — the shipped invocation is the node exec form, so the shebang is decorative and no runtime failure is known; the exposure is direct POSIX execution, Wave 6 determinism, and a policy file asserting a stale rationale |
