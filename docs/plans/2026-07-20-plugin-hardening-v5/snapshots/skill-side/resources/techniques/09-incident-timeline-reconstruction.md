...
## Relationship to the Core Log Template

The core log template in SKILL.md captures phase-level timestamps (`T_START`,
`T_CATEGORIZED`, `T_ISSUE_LOCATED`, etc.). This technique extends those raw
timestamps into a full incident timeline that includes events BEFORE
troubleshooting started: when the bug was introduced, when it was first
noticed, when monitoring caught it.

For SEV1/SEV2 incidents, build the full timeline. For SEV3/SEV4, the
phase-level timestamps in the core log are usually sufficient.
