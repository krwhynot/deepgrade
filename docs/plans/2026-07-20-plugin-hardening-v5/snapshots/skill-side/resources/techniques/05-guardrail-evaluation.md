...
The difference between "Prevention" (which the core methodology has) and Guardrail Evaluation is specificity. "Prevent this in the future" is vague. "LINT-05 doesn't check for null returns from this API, and the test suite has no integration test for the payment→receipt flow" is actionable.
...
|---------------|-------|---------|--------|
| **Not present** | `not-present` | No test/rule exists for this scenario | Write the test or add the rule |
| **Present but insufficient** | `insufficient` | Test exists but doesn't cover this case | Expand test coverage for this case |
| **Present but disabled** | `disabled` | Rule exists but is disabled or skipped | Re-enable and understand why it was disabled |
| **Present but wrong** | `wrong` | Test exists but asserts the wrong thing | Fix the test assertion |
| **Present and passed** | `wrong-layer` | Test ran but the bug is at a different layer | Add coverage at the correct layer |
| **Not applicable** | `n-a` | No reasonable guardrail could catch this | Document as accepted risk |

Use machine-friendly tokens in the format `{guardrail-type}:{classification}`. Examples: `unit-tests:not-present`, `ci:insufficient`, `linter:disabled`, `types:n-a`.
...
```markdown
**Guardrails missed:** unit-tests:not-present, ci:insufficient
...
Guardrail evaluation runs AFTER Phase 4 (Fix) is complete and verified. The fix is already in place; this is analysis, not more fixing. This step generates RECOMMENDED follow-up actions — it does not automatically apply additional edits.

Recommended for: every fixed bug that reached an environment beyond local development (staging, production, customer-facing).
Optional for: trivial local bugs caught during development.
