#!/usr/bin/env node
// DeepGrade: SessionStart handler — lane N
//
...
//
// TWO BUGS in dg-session-start.sh that a naive port would have carried, both from
...
if (!status) {
  ...
  emit(`[DeepGrade] Active plan: ${latest.name} — status.json is present but not valid JSON.`);
}
...
const report = 'docs/audit/deepgrade-report.md';
try {
...
if (source === 'compact') {
  ...
  parts.push(`Resuming after compaction. Continue with /deepgrade:plan ${latest.name}`);
}

emit(`[DeepGrade] ${parts.join(' ')}`, `Active DeepGrade plan: ${latest.name}, phase ${phase} (${phaseStatus}).`);
