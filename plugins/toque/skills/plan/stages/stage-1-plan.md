...
INTENT-ONLY MODE:
When invoked as `/toque:plan intent {name}`, run this stage only. Complete
...
given in the same session; the owner resumes with `/toque:plan {name}` when
...
`__web_fetch_exa`, or `__perplexity_ask` (see the `toque:mcp-research` skill —
...
If [1]:
  ...
  - In intent-only mode: report "Intent accepted and committed. Resume with
    /toque:plan {name} to begin design." and STOP.
