...
The loop targets **36/40** (upper GREEN threshold from Toque's plan-auditor rubric):
...
4. If no plan found, display:
   ...
   1. `/toque:codex-challenge {plan-name}` — review a specific plan
   2. `/toque:codex-challenge docs/specs/my-spec.md` — review any spec file
   3. Create a plan first with `/toque:plan` or `/toque:quick-plan`"
...
- If the plan has not been through Phase 5 (Audit), suggest:
  ...
  /toque:quick-audit first, then /toque:codex-challenge."
</constraints>
...
/toque-audit:codebase-gates, /toque-audit:codebase-security,
/toque-readiness:readiness-generate, /toque-readiness:readiness-scan
</valid_commands>
