...
description: |
  ...
  Called by /deepgrade-audit:codebase-gates.
model: sonnet
color: orange
...
# Generated by /deepgrade-audit:codebase-gates from audit data.
...
Generated by /deepgrade-audit:codebase-gates.
...
" 2>/dev/null)
  ...
  if echo "$filepath" | grep -qiE '\.(config|env)|appsettings|migration'; then
    echo "NOTE: Config/migration file changed ($filepath). Consider running /deepgrade-audit:codebase-delta to check for baseline drift." >&2
  fi
  if echo "$filepath" | grep -qiE 'auth|credential|secret|password|token|ssl|cert'; then
    echo "NOTE: Security-related file changed ($filepath). Consider running /deepgrade-audit:codebase-security." >&2
  fi
  ...
  if [ "$count" -ge "$THRESHOLD" ] && [ "$((count % THRESHOLD))" -eq 0 ]; then
    ...
    echo "Consider running: /deepgrade-audit:codebase-delta" >&2
    echo "==========================" >&2
    echo "" >&2
  fi
}
...
if days > $STALE_DAYS:
  print(f'Last audit was {days} days ago. Consider running /deepgrade-audit:codebase-delta.', flush=True)
...
if d.get('security_files_changed'):
  print('Security-related files were modified. Consider /deepgrade-audit:codebase-security.', flush=True)
...
When the user runs /deepgrade-audit:codebase-delta or /deepgrade-audit:codebase-audit, the tracker should
...
- Baseline tracker configuration:
  ...
  - Auto-reset: running /deepgrade-audit:codebase-delta or /deepgrade-audit:codebase-audit resets the tracker
- Three-layer summary:
