...
description: |
  ...
  pre-commit hooks that enforce Toque audit findings. Creates
  ...
  Called by /toque-audit:codebase-gates.
model: sonnet
color: orange
...
<context>
The Toque audit (Phase 2) produces risk-assessment.md and
toque-report.md. These contain HIGH-risk modules, CRITICAL findings,
...
- docs/audit/toque-report.md (severity-classified findings)
...
Create .github/workflows/toque-gate.yml:
...
```bash
#!/bin/bash
# Toque: Do-Not-Touch Zone Guard
# PreToolUse hook for Write|Edit. Warns on protected files, allows override.
# Generated by /toque-audit:codebase-gates from audit data.
...
cat << EOF
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "DO-NOT-TOUCH ZONE: $MATCHED_REASON. File: $FILE_PATH. This file is in a protected zone from the Toque audit. You can approve to override (the edit will be logged) or deny to cancel."
  }
}
EOF
...
Generated by /toque-audit:codebase-gates.
...
get_last_audit_date() {
  if [ -f "docs/audit/toque-report.md" ]; then
    stat -c %Y "docs/audit/toque-report.md" 2>/dev/null || \
    stat -f %m "docs/audit/toque-report.md" 2>/dev/null || echo "0"
  else
    echo "0"
  fi
}
...
" 2>/dev/null)
  ...
  if echo "$filepath" | grep -qiE '\.(config|env)|appsettings|migration'; then
    echo "NOTE: Config/migration file changed ($filepath). Consider running /toque-audit:codebase-delta to check for baseline drift." >&2
  fi
  if echo "$filepath" | grep -qiE 'auth|credential|secret|password|token|ssl|cert'; then
    echo "NOTE: Security-related file changed ($filepath). Consider running /toque-audit:codebase-security." >&2
  fi
  ...
  if [ "$count" -ge "$THRESHOLD" ] && [ "$((count % THRESHOLD))" -eq 0 ]; then
    ...
    echo "Consider running: /toque-audit:codebase-delta" >&2
    echo "==========================" >&2
    echo "" >&2
  fi
}
...
if days > $STALE_DAYS:
  print(f'Last audit was {days} days ago. Consider running /toque-audit:codebase-delta.', flush=True)
...
if d.get('security_files_changed'):
  print('Security-related files were modified. Consider /toque-audit:codebase-security.', flush=True)
...
When the user runs /toque-audit:codebase-delta or /toque-audit:codebase-audit, the tracker should
...
- Baseline tracker configuration:
  ...
  - Auto-reset: running /toque-audit:codebase-delta or /toque-audit:codebase-audit resets the tracker
- Three-layer summary:
