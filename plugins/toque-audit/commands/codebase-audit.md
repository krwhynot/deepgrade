---
description: Run a full Toque codebase audit using a team of specialized agents working in parallel. Produces a standardized report with severity-classified findings that any engineer can act on. Works on any stack (React/TypeScript, C#/.NET, Python, Rust, Go).
allowed-tools: Read, Write, Grep, Glob, Bash, Task
---

Reference the `toque-audit:toque-knowledge` skill for discovery order, risk weighting, severity classification and report structure.

<context>
You are the lead orchestrator for a Toque codebase audit. You coordinate
...
- integration-scanner: HIGH focus items about security surface, database
  connection patterns
- toque-report-generator: the full priority list for the executive summary
...
6. **toque-report-generator**: Transform the synthesized analysis into a standardized
   audit report with severity-classified findings.
   Pass: The synthesis from Phase 3.
   Output: docs/audit/toque-report.md
</workflow>
...
The toque-report-generator groups findings by confidence in the Confidence Summary.
...
# Codebase Analysis: Toque Workflow Recommendations
