...
| `/deepgrade:codex-challenge` | Adversarial review: Codex scores your plan (8 dimensions, max 40), Claude optimizes until 36/40 GREEN. |
...
| Command | What It Does |
|---------|-------------|
| `/deepgrade-readiness:readiness-scan` | AI Readiness scan. 52 checks, 9 categories, letter grade A+ to F. |
| `/deepgrade-readiness:readiness-generate` | Generate missing artifacts found by readiness scan. |

### Codebase Audit (Phase 2: What's in it and what's risky?)

| Command | What It Does |
|---------|-------------|
| `/deepgrade-audit:codebase-audit` | Full DeepGrade codebase audit. 6 parallel agents: features, dependencies, docs, risk, integrations, report. |
...
| Command | What It Does |
|---------|-------------|
| `/deepgrade-audit:codebase-delta` | Quick re-measurement. What improved, what regressed, KPI dashboard. |
...
1. `/deepgrade-readiness:readiness-scan` to get your baseline score
2. `/deepgrade-readiness:readiness-generate` to fix low-hanging issues
3. Re-scan to verify improvement
4. `/deepgrade-audit:codebase-audit` when score reaches 80%+
5. `/deepgrade-audit:codebase-gates` to generate CI quality checks
...
- `/deepgrade-audit:codebase-characterize [module]` to generate golden master tests
