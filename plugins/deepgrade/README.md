...
`deepgrade-readiness` (AI-readiness grading) and `deepgrade-audit` (codebase
audits) — install any subset.
The full reference is [GUIDE.md](GUIDE.md).
...
| Command | Description |
| ------- | ----------- |
| `/deepgrade:plan` | Six-stage playbook workflow: Plan, Design, Build, Test, Deploy, Maintain. Human gate at every stage |
| `/deepgrade:quick-plan` | Lightweight plan for small changes |
| `/deepgrade:plan-status` | Check plan progress and stage status |
...
| Command | Description |
| ------- | ----------- |
| `/deepgrade:quick-audit` | Audit any technical plan or spec through 8 review dimensions with evidence |
...
| Command | Description |
| ------- | ----------- |
| `/deepgrade:documentation` | Generate ADRs, BRDs, PRDs, READMEs, runbooks, release notes, specs |
| `/deepgrade:quick-cleanup` | Clean a folder of messy documents into reference material |
| `/deepgrade:troubleshoot` | 4-phase debugging framework with incident triage, containment, status updates, and postmortems |
...
- **`dg-canary.js`** checks the auditor. Before the audit runs, it injects one
  ...
  revision loop.
- **`dg-evidence-validate.js`** checks the evidence. It re-reads every cited
...
**Optional:** the OpenAI Codex CLI on PATH, for `/deepgrade:codex-challenge`
...
- **3 hook handlers** - `scripts/dg-session-start.js`, `dg-subagent-stop.js`, `dg-pre-compact.js`
- **2 design-gate tools** - `scripts/dg-canary.js` and `dg-evidence-validate.js`, invoked by Stage 2 of `/deepgrade:plan`, not by hooks
